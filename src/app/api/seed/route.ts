  ...
  try {
    ...
    for (const t of tutoresData) {
      const tutorUser = await db.user.upsert({
        ...
        update: {},
        create: {
          name: t.name, email: t.email, password: tutorHash,
          role: 'tutor', universidad: t.universidad, carrera: t.carrera,
        },
      })
      ...
      if (!existingProfile) {
        const profile = await db.tutorProfile.create({
          data: {
            userId: tutorUser.id, nombreCompleto: t.name,
            universidad: t.universidad, carrera: t.carrera,
            anioIngreso: t.anioIngreso, precioPorHora: t.precioPorHora,
            modalidad: t.modalidad, estado: t.estado,
            calificacion: t.calificacion, totalResenas: t.totalResenas,
          },
        })
        for (const r of t.ramos) {
          await db.tutorRamo.create({
            data: {
              tutorId: profile.id, nombreRamo: r.nombreRamo, nota: r.nota,
              profesor: r.profesor, anio: r.anio, semestre: r.semestre,
              fueAyudante: r.fueAyudante, materialDisp: JSON.stringify(r.materialDisp),
            },
          })
        }
      }
    ...
    // Reviews
    const sebastian = await db.tutorProfile.findFirst({ where: { user: { email: 'sebastian.morales@uc.cl' } } })
    const matias    = await db.tutorProfile.findFirst({ where: { user: { email: 'matias.silva@uchile.cl' } } })
    ...
    if (!existingMaterial) {
      ...
      if (sebUser) {
        await db.materialItem.createMany({ data: [
    ...
    return NextResponse.json({
      success: true,
      message: '✅ Base de datos cargada con datos de demo',
      credentials: {
        admin:   'admin@mentora.cl / admin123',
        alumno:  'maria.gonzalez@uc.cl / alumno123',
        tutor1:  'sebastian.morales@uc.cl / tutor123',
        tutor2:  'camila.torres@usach.cl / tutor123',
        tutor3:  'matias.silva@uchile.cl / tutor123',
      },
    })
  } catch (error) {
